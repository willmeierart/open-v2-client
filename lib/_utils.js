...
export const FBdataTransformer = data => {
  ...
  const transformedGalleries = data.sort((a, b) => a.name - b.name).reduce((a, b) => {
    ...
    const { id, name, cover, about, location, mission } = body
    const source = cover ? cover.source : null

    // b.events.data.forEach(event => {
    //   console.log(event)
    //   const { id, cover: { source }, description, start_time, end_time, place, name, owner } = event
    //   if (test) {
    //     transformedEvents.upcoming.push(
    //       { id, type: 'event', name, cover: source, description, start_time, end_time, place, owner: owner.name }
    //     )
    //     transformedEvents.today.push(mockPointerEvent)
    //   }
    // })
    a.push({ id, type: 'gallery', name, cover: source, about, location, mission })
    return a
  }, [])
  const retObj = {
...
export const makeMarker = marker => {
  ...
  const randMarker = Os[Math.floor(Math.random(Os.length))]
  const mainMarker = Os[3]
  return marker ? {
    id: marker.id,
    name: marker.name,
    marker: {
      ...
      // icon: { url: marker.active ? `/static/assets/ox${Math.floor(Math.random() * 4) + 1}.svg` : `/static/assets/o${Math.floor(Math.random() * 4) + 1}.svg` },
    },
    type: marker.type
  } : {}
}
