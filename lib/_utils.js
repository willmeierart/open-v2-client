...
export const FBdataTransformer = data => {
	...
	const transformedGalleries = data.sort((a, b) => a.name - b.name).reduce((a, b) => {
		...
		const { id, name, cover, about, location, mission, emails, description, link, website, photos, general_info } = body
		let ABOUT = about && description ? (about.length > description.length ? about : description) : about || description
